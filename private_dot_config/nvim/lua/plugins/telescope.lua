    ...
    return function(line)
        ...
        entry.display = function(et)
            ...
            if vim.fn.isdirectory(et.path) == 1 then
                ...
                icon, icon_hl = devicons.get_icon(filename, vim.fn.fnamemodify(filename, ":e"),
                    { default = true })
            end
...
local search_handlers = {
    ...
    projects = function()
        ...
        local open_with_oil = function(prompt_bufnr)
            ...
            if selection.cwd then path = vim.fs.joinpath(selection.cwd, path) end
        ...
        config = function()
            ...
            require("project").setup({
                ...
                patterns = {
                    ...
                    "Makefile", "package.json", "Pipfile", "pyproject.toml", ".nvim.lua",
                },
            ...
            telescope.setup({
                defaults = {
                    file_ignore_patterns = {
                        ...
                        "__pycache__/", "%.o", "%.a", "%.out", "%.bin",
                    },
                    mappings = {
                        i = { ["<C-h>"] = function() vim.api.nvim_input("<C-w>") end },
                    },
                },
                pickers = {
                    find_files = {
    ...
    },
    {
        ...
        opts = {
            dashboard = {
                ...
                preset = {
                    keys = {
                        { icon = "󰈔 ", key = "<leader>sf", desc = "Search Files", action = search_handlers.find_files },
                        { icon = " ", key = "<leader>sg", desc = "Search by Grep", action = search_handlers.live_grep },
                        { icon = " ", key = "<leader>sp", desc = "Search Projects", action = search_handlers.projects },
                        {
                            icon = " ",
                            key = "<leader>pf",
                            desc = "Open Oil",
                            action = function()
                                local function get_root()
                                    ...
                                    if status then
                                        return project.get_visual_configs() or project.find_pattern_root() or
                                            vim.fn.getcwd()
                                    end
                        ...
                        { icon = "󰞋 ", key = "<leader>sh", desc = "Search Help", action = search_handlers.help_tags },
                        { icon = "󰘳 ", key = "<leader>sk", desc = "Search Keymaps", action = search_handlers.keymaps },
                        {
                            icon = "󱎘 ",
                            key = ":q",
                            desc = "Quit",
                ...
                sections = {
                    function()
                        ...
                        if #lines == 0 then
                            return { { text = "  Welcome to Neovim", hl = "SnacksDashboardHeader", align = "center" } }
                        end
